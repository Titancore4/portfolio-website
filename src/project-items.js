...
const PROJECT_ITEMS = [
  {
    title: "Notes App",
    text: [
      "This project is a fully functional notes management web application built using React with a strong emphasis on modern React Hooks, particularly the useState hook, to manage application state efficiently. The application allows users to create, edit, and delete notes in a seamless and intuitive interface.",

      "The project is designed to showcase how dynamic data flows through a React application without relying on class components. Each note is handled as part of the application’s state, enabling real-time updates and ensuring that changes are immediately reflected in the user interface.",

      "Special attention was given to user experience and code structure. The application is organized into reusable components, making the codebase easy to maintain, extend, and scale. Event handling is implemented thoughtfully to ensure smooth interactions while maintaining a responsive and consistent UI.",
...

  {
    title: "Blog Platform",
    text: [
      "This project is a blog web application developed using Python and Flask, designed to demonstrate full-stack web development concepts with a focus on backend logic, authentication, and data persistence. The application allows users to browse blog posts, read content, and participate in discussions through a commenting system.",

      "The platform uses MongoDB as its primary database, storing blog posts, user information, and comments in a structured and scalable manner. An authentication system is implemented using sessions and cookies, enabling secure user login and access control throughout the application.",

      "Role-based access control is a core feature of the system. While registered users can comment on posts, administrative privileges are required to create new blog entries or edit existing content. This separation of permissions highlights proper authorization handling and secure content management practices.",
